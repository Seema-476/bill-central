    ...
    return (
        <div className="bg-black bg-opacity-80">
            <div className="max-w-[1358px] flex items-center justify-between py-5 max-lg:px-5 px-4 mx-auto w-full">
                <a className='lg:max-w-[286px] max-w-[150px]' href="/"> <img src="/assets/images/webp/nav-logo-gilded.webp" alt="main-logo" /> </a>
                <div className={`flex gap-[29px] max-xl:gap-5 max-xl:fixed max-xl:top-0 max-xl:-right-full max-xl:h-full max-xl:w-full max-xl:justify-center max-xl:items-center transition-all duration-300 max-xl:flex-col max-xl:bg-black max-xl:z-10 ${open ? 'max-xl:!right-0' : ''}`}>
                    <div className='flex gap-10 max-xl:gap-6 max-xl:flex-col items-center' >
                        {HEADER_LIST.map((item, index) => (
                            <a onClick={closeHandler} href={item.link} key={index} className="font-Josefin text-white !whitespace-nowrap text-[22px] font-normal text-stroke transition-all duration-300">{item.title}</a>
                        ))}
                    </div>
                    <button onClick={closeHandler} className='text-white font-Josefin cursor-pointer hover:scale-110 transition-all duration-500 flex gap-2.5 items-center text-2xl leading-[24px] rounded-[79px] py-3.5 px-[34px] common-button shadow-custom-3xl'><Discord />Discord</button>
                </div>
