          ...
          <div className='lg:w-[50%] w-full px-3'>
            ...
            <div className='!duration-700 !transition-all !ease-in-out'>
              {COMPREHENSIVE_DETAILS.map((obj, index) => (
                <div key={index} onClick={() => toggleAccordion(index)} class={`border border-gray bg-white hover:!shadow-custom-xl !transition-all !ease-in-out !duration-700 rounded p-[14px] mb-6 cursor-pointer  ${accoActive === index ? 'shadow-custom-xl' : ''}`}>
                  <div className='flex gap-6 items-center'>
                    <span>{obj.comparisonsSvg}</span>
                    <div className='flex flex-col w-full'>
                      <div className={`flex ${accoActive !== index ? '!justify-between' : 'justify-between'}`}>
                        <p className='font-normal text-2xl leading-custom-6xl text-dark-blue pb-2'>{obj.question}</p>
